    ...
    <div className="min-h-screen bg-background overflow-x-hidden">
      ...
      <section className="pt-32 md:pt-40 pb-10">
        <div className="mx-auto max-w-4xl px-5 text-center">
          ...
          <h1 className="mt-2 text-4xl md:text-6xl font-bold leading-tight">
            We'd love to <span className="relative text-[var(--coral)]">hear from you
              <Squiggle className="absolute -bottom-2 left-0 w-full text-[var(--sunny)]" />
            </span>
          </h1>
          <p className="mt-5 text-lg text-muted-foreground">Questions about enrollment, tours or just curious? Reach out — we'll reply in less than 24 hours.</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-5 grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {cards.map((c, i) => (
            <div key={i} className="bg-card rounded-3xl p-6 border border-border">
              <div className="h-12 w-12 rounded-2xl grid place-items-center text-white" style={{ background: c.color }}>
      ...
      <section className="py-12">
        <div className="mx-auto max-w-7xl px-5 grid lg:grid-cols-2 gap-10">
          <form onSubmit={submit} className="bg-card rounded-3xl border border-border p-8 space-y-5">
            <h2 className="text-2xl font-bold">Send a message</h2>
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                <Label htmlFor="name">Parent Name</Label>
                <Input id="name" value={form.name} onChange={update("name")} className="mt-1 h-11 rounded-xl" />
                {errors.name && <p className="text-xs text-destructive mt-1">{errors.name}</p>}
              </div>
              <div>
                <Label htmlFor="email">Email</Label>
                <Input id="email" type="email" value={form.email} onChange={update("email")} className="mt-1 h-11 rounded-xl" />
                {errors.email && <p className="text-xs text-destructive mt-1">{errors.email}</p>}
              </div>
              <div>
                <Label htmlFor="phone">Phone</Label>
                <Input id="phone" value={form.phone} onChange={update("phone")} className="mt-1 h-11 rounded-xl" />
                {errors.phone && <p className="text-xs text-destructive mt-1">{errors.phone}</p>}
              </div>
              <div>
                <Label htmlFor="subject">Subject</Label>
                <Input id="subject" value={form.subject} onChange={update("subject")} className="mt-1 h-11 rounded-xl" />
                {errors.subject && <p className="text-xs text-destructive mt-1">{errors.subject}</p>}
              </div>
            </div>
            <div>
              <Label htmlFor="message">Message</Label>
              <Textarea id="message" value={form.message} onChange={update("message")} rows={5} className="mt-1 rounded-xl" />
              {errors.message && <p className="text-xs text-destructive mt-1">{errors.message}</p>}
            </div>
            <Button type="submit" disabled={submitting} className="w-full rounded-full h-12 bg-[var(--coral)] hover:bg-[var(--coral)]/90 text-white font-semibold">
